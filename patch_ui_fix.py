...
css='''
.app-header{position:sticky;top:0;z-index:60;background:rgba(11,22,32,.98);backdrop-filter:blur(12px);border-bottom:1px solid #253747;padding:14px 16px 16px!important;display:flex!important;justify-content:center!important;align-items:center!important}
...
.app-header-logo{display:block;width:68px;height:68px;border-radius:17px;object-fit:cover;box-shadow:0 0 0 1px #31506a,0 8px 24px rgba(0,0,0,.30)}
.brand-wordmark{display:block;width:255px;max-width:76vw;height:auto;aspect-ratio:507/69;object-fit:contain;margin:3px auto 1px}
.app-meta{display:flex;align-items:center;justify-content:center;gap:10px;flex-wrap:wrap;margin-top:2px}
...
@media(max-width:760px){
 ...
 .app-header-logo{width:66px;height:66px}
 .brand-wordmark{width:245px;max-width:74vw}
 .app-meta{gap:8px}
...
new_header=f'''<header class="app-header"><div class="app-brand-stack"><img class="app-header-logo" src="{logo_src}" alt=""><img class="brand-wordmark" src="/investiga-wordmark.svg?v=2" alt="INVESTIGA"><div class="app-meta"><span class="muted">AI Investigation Simulator</span><span id="version" class="tag">...</span></div></div></header>'''
s=re.sub(r'<header(?: class="app-header")?>.*?</header>',new_header,s,count=1,flags=re.S)

p.write_text(s,encoding='utf-8')
print('Fixed wordmark rendering with exact asset and balanced mobile header')
